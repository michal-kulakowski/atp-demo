...
#tar -cvzf vpn_scenario.tar.gz demo vpn_demo.sh /opt/phoenix/config/discoverFile/
#tar -xvzf vpn_scenario.tar.gz
...
crontab /root/defaultCron.txt
cat /root/demo/2017/scenario1/cron.out >> /opt/phoenix/config/sys/var/spool/cron/root
...
echo ""
echo "Copying files for Discovery"
cp /root/opt/phoenix/config/discoverFile/* /opt/phoenix/config/discoverFile/
chown admin.admin /opt/phoenix/config/discoverFile/*
chmod +r /opt/phoenix/config/discoverFile/*
echo ""
echo "Next steps:"
echo "1) You need to discover the devices 10.10.100.27, 192.168.3.1. Do this by creating an SNMP \
credential, associate that credential to the IP addresses then create a discovery. When creating \
